...
| OR | 2020 | **Socrata `data.oregon.gov/ijbz-jpx8` verified 2026-07-09**: official WARN dataset, 397 rows 2020-03→2026-05 with counts + received/layoff dates, ~2-month update lag. The live HECC tracker (`ccwd.hecc.oregon.gov/Layoff/WARN`) has gone near-empty (Multnomah shows 2 rows) — prod is missing most recent notices (2024: 3 vs 71; 2026: 1 vs 33) despite `ok` runs | 2020-03 (Socrata; HECC retains six years but publishes almost nothing) | **backfill from Socrata pending** (+~300 rows; watch cross-source dedup — `company_name` is sometimes a facility name); keep HECC scraper for freshness; pre-2020 → inquiry |
