...
| IL | ~~2025~~ **2020 ✅** | `illinoisworknet.com/LayoffRecovery/` archive — monthly XLSX 2020+, monthly **PDFs 1999–2019** (a two-column labeled *form*, not a table — each flattened line split at its right-column label, x-independent) | **1999** (PDF era) | xlsx era **done 2026-06-12** (+861 on rerun); **PDF-era parser done 2026-07-07** (`parse_il_pdf`, wired into the IL backfill spec alongside the xlsx discovery). Dry-run: ~2,724 net-new rows, near_miss=0; Jan 2019 is image-only (OCR gap). Gated real Job pending |
