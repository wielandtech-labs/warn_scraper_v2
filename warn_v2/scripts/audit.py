...
BLOCKED = frozenset({"AR", "NH", "OK", "TN", "WY"})

# Approximate state bounding boxes (lat_min, lat_max, lon_min, lon_max) with ~1
# degree padding.  Used only to catch *gross* geocoding errors (e.g. a GA notice
# landing in California, or at lat/lon 0), so generous bounds are intentional.
...
_REMEDY = {
    ...
    "row_drift": "heal --state {st}",
    "scraper_fetch_failed": "heal --state {st}",
    "scraper_parse_failed": "heal --state {st}",
