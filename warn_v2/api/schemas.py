...
class CompanyOut(BaseModel):
    ...
    # Null on canonical rows. Just a pointer — not the provider data — so safe to expose.
    ...
    # exposed here. They are stored for internal use only; publishing the provider-sourced
...
class CompanyEnrichedOut(CompanyOut):
    """CompanyOut + the provider enrichment fields. Served only to paid/admin sessions.

    Anonymous and free-tier responses use CompanyOut, so these keys are absent
    (not null) for them — the public shape never includes the provider-sourced fields.
    """
