...
/** Estimated occupation mix for a notice, from the national OEWS staffing
 *  pattern of the company's NAICS industry. Renders nothing when no pattern
 *  is available (no NAICS, or an industry OEWS doesn't cover). Plain CSS
 *  bars — no recharts, so notice-detail stays out of the chart chunk. */
export function OccupationMix({ noticeId }: { noticeId: string }) {
  ...
  return (
    <div className="card">
      <h2 className="mb-2 text-lg font-semibold">Estimated occupation mix</h2>
      <ul className="space-y-1.5">
        {mix.occupations.map((o) => (
          <li key={o.soc_code} className="flex items-center gap-2 text-sm">
