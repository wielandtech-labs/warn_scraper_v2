 ...
 *  "N projected (M to date)" on the projected point. contentStyle/labelStyle
 *  arrive from the host <Tooltip>, which clones this element with its props. */
export function ProjectionTooltip({
  ...
  const entries = payload.filter((e) => {
    if (e.value == null) return false; // the nulled solid series on the projected point
